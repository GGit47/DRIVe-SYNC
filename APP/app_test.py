...
.kpi-val {{ font-family:var(--mono); font-size:18px; font-weight:500; line-height:1; }}
.kpi-lbl {{ font-size:8px; letter-spacing:2px; text-transform:uppercase;
...
with st.sidebar:
    ...
    dot_obd = GREEN if OBD_MODEL_LOADED else AMBER
    dot_beh = GREEN if BEH_MODEL_LOADED else AMBER
    lbl_m   = "Telematics loaded"    if MODELS_LOADED    else "Telematics: synthetic"
    lbl_obd = "OBD model loaded"     if OBD_MODEL_LOADED else "OBD: synthetic"
    lbl_beh = "Behaviour model loaded" if BEH_MODEL_LOADED else "Behaviour: synthetic"
    st.markdown(f"""
    <div class="status-row"><div class="dot" style="background:{dot_m}"></div>{lbl_m}</div>
    <div class="status-row"><div class="{"dot-live" if OBD_MODEL_LOADED else "dot"}" style="background:{dot_obd}"></div>{lbl_obd}</div>
    <div class="status-row"><div class="dot" style="background:{dot_beh}"></div>{lbl_beh}</div>
...
# ─────────────────────────────────────────────────────────────
    ...
    with aL:
        ...
        for cat, detail in [
            ("ML",      "scikit-learn, XGBoost, Random Forest — Ensemble voting"),
