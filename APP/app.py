...
# Live OBD reader — connects to ELM327 emulator via TCP
# Graceful: if obd_live_reader.py is missing the app still works
try:
    from obd_live_reader import get_live_reading, is_connected as obd_is_connected
    OBD_LIVE_AVAILABLE = True
except ImportError:
    OBD_LIVE_AVAILABLE = False
    def get_live_reading(): return {}
    def obd_is_connected(): return False
...
with st.sidebar:
    ...
    # ── Discreet theme switch ──────────────────────────────────
    _theme_label = "Light mode" if _IS_DARK else "Dark mode"
    _knob_pos    = "15px"       if _IS_DARK else "2px"
    _track_bg    = "#333"       if _IS_DARK else "#C0392B"
    st.markdown(f"""
    <div class="theme-toggle" id="theme-wrap">
        <div class="toggle-track" style="background:{_track_bg}">
            <div class="toggle-knob" style="left:{_knob_pos}"></div>
        </div>
        <span style="color:{SILVER};font-size:9px;letter-spacing:1.5px">{_theme_label}</span>
    </div>""", unsafe_allow_html=True)
    if st.button("Switch theme", key="theme_btn",
...
# ─────────────────────────────────────────────────────────────
    ...
    for tick in range(30 if live else 1):
        # Use live emulator data if connected, synthetic fallback otherwise
        if OBD_LIVE_AVAILABLE and obd_is_connected():
        ...
        with placeholder.container():
            ...
            st.markdown(f'<div style="font-size:9px;letter-spacing:1px;color:{BORDER};text-align:right">'
                        f'Last update: {reading["timestamp"]} — {"LIVE (TCP:35000)" if reading.get("source")=="live" else "synthetic"}</div>',
                        unsafe_allow_html=True)
        if live:
            time.sleep(1.5)
